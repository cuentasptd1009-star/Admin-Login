      ...
      {(() => {
        ...
        return (
          ...
          >
            ...
            {/* Search bar + mic inline */}
            <div className="px-3 pb-3">
              <div className={`flex items-center rounded-xl border bg-white/7 overflow-hidden transition-all duration-150 ${isListening ? 'border-red-500/50' : isSbFocused('search') ? 'border-white/30 ring-2 ring-primary/60 bg-white/12' : 'border-white/10'}`}>
                <button
                ...
                >
                  ...
                  <span className={`flex-1 text-left truncate ${searchQuery ? 'text-white' : 'text-white/30'}`}>
                    {searchQuery || (isListening ? 'Escuchando...' : 'Buscar...')}
                  </span>
                  {searchQuery && (
                    <button onClick={(ev) => { ev.stopPropagation(); setSearchQuery(''); }} className="text-white/30 hover:text-white flex-shrink-0">
                ...
                {voiceSupported && (
                  <button
                    ...
                    className={`flex-shrink-0 p-2.5 border-l border-white/10 transition-all duration-150
                      ${isListening ? 'text-red-400 bg-red-500/15' : 'text-white/35 hover:text-white hover:bg-white/10'}
                      ${isSbFocused('mic') ? 'ring-2 ring-primary/60 bg-white/15 text-white' : ''}`}
