...
import { Play, ArrowLeft, Film, Tag, Search, X, Lock, Heart, Info, Star } from 'lucide-react';
...
export default function MovieDetail() {
  ...
  }, [mvZone, btnIndex, gridRow, gridCol, related, actionButtons, isFav]);
    ...
    <div className="min-h-screen bg-[#141414] text-white flex flex-col select-none">
      ...
      <div className="relative w-full" style={{ minHeight: '70vh', maxHeight: '85vh' }}>
        ...
        <div className="absolute bottom-0 left-0 p-6 sm:p-10 md:p-14 max-w-2xl space-y-3 sm:space-y-4">
          ...
          {mvZone === 'buttons' && (
            <p className="text-white/30 text-xs pt-1">◀▶ Navegar · Enter Seleccionar · ▼ Películas</p>
          )}
        </div>
      </div>
      ...
      {(movies?.length ?? 0) > 1 && (
        <section className="px-4 sm:px-8 md:px-14 py-8 space-y-5">
          <div className="flex flex-col sm:flex-row sm:items-center gap-3">
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className={`px-3 py-1 rounded-full text-xs font-medium transition-all ${!filterCat ? 'bg-orange-500 text-white' : 'bg-white/10 text-white/50 hover:text-white hover:bg-white/15'}`}
              >
                Todas
              </button>
              {categories.map(cat => (
                <button
                  ...
                  className={`px-3 py-1 rounded-full text-xs font-medium transition-all ${filterCat === cat ? 'bg-orange-500 text-white' : 'bg-white/10 text-white/50 hover:text-white hover:bg-white/15'}`}
                >
                  {cat}
                </button>
              ))}
          ...
          {related.length === 0 ? (
            <p className="text-white/30 text-sm py-8 text-center">No hay más películas{search ? ` con "${search}"` : ''} en esta categoría</p>
          ) : (
            <>
              {mvZone === 'grid' && (
                <p className="text-white/30 text-xs">◀▶▲▼ Navegar · Enter Ver · Esc Volver</p>
              )}
