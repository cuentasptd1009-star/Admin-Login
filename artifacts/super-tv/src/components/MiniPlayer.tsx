...
export function MiniPlayer() {
  ...
  function buildProxyUrl(ch: { id: number; streamUrl: string }): { url: string; streamFormat: string } {
    ...
    const isHls = lower.endsWith('.m3u8') || lower.includes('/hls/');
    if (isHls) {
    ...
    const streamFormat = isDash ? 'dash' : isFlv ? 'flv' : 'native';
    return { url: `/api/channels/${ch.id}/stream?token=${encodeURIComponent(token)}`, streamFormat };
  }
